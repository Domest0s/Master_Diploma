\documentclass[pdftex,14pt]{scrartcl}
%dvips
...
\newcommand{\thesistopicUA}
{Початкова розробка дизайну дрейфових трубок для детектору в експерименті SHiP}

\newcommand{\thesistopicRU}
{Начальная разработка дизайна дрейфовых трубок для детектора в эксперименте SHiP}

\newcommand{\thesistopicEN}
{Preliminary design studies of a drift tube detector for SHiP experiment}
...
%	A variation of the signal height introduces a variation in the time when the signal passes the threshold and is considered to be the main contribution to the STRAW tracker resolution.
	...
	\end{tikzpicture} 
	\caption{Схематичне зображення факту проходження зарядженої частинки через дрейфову трубку. Місця утворення іон-електронних пар вздовж треку зображено оранжевими кружечками. $r_{track}$ - відстань від місцеположення найближчих кластерів до дроту. $r_{tube} = 4.9 mm$ - радіус трубки.}
%	\caption{Schematic view of a particle passing the straw and producing ionization clusters(orange points). The ionization cluster electrons drift to the wire and induce the signal. The closest distance from the track to the wire, $r_{track}$, and radius of the straw, $r_{tube} = 2.45 mm$, are also indicated. }
...
%	Ефективність реєстрації треків в області периферії трубки від рівномірного опромінення 50 тис. треків
	...
	\begin{figure}[h!]
		\centering
		\subfloat[кількість електронів на кластер]{
			\includegraphics[width=0.45\textwidth]{el_per_cluster} 
			\label{fig:el_per_cluster} }%
		\qquad
		\subfloat[кількість кластерів на трек]{
...
%	GARFIELD can provide us by amplification factor $G$ for any point of the tube(because  $G$ is coordinate dependent magnitude). The amplification factor is equal almost in whole tube space except neighbourhood near the wire because electric field  becomes significantly high only near the wire (see figs \ref{fig:elFieldCentered}, \ref{fig:elField1mmShifted}). When the wire is shifted from the center of the cube the electric field in area close to the wire is the same as in centered state. So the amplification factor $G$ is quite similar in both cases.
	...
	\begin{figure}[!h] 
		\centering
		\subfloat[центрований дріт]{
			\includegraphics[width=0.45\textwidth]{fieldCentered} 
			\label{fig:elFieldCentered} }%
		\qquad
		\subfloat[дріт зміщений на $1 mm$ відносно центру трубки]{
...
%		\node[] at (2.5,2.5) {$\nearrow$};
	...
	\end{tikzpicture}
	\caption{Схема циркуляції газу та контролю тиску, і потоку газу через трубку}
	%\caption{Gas circulation scheme and circuit of connection of flow control through the drift tube}
	\label{fig:gasFlowScheme}
	\end{figure}		
	
	Електрична схема підключення трубки показана на Рис.\ref{fig:electricCircuit}.
...
\begin{thebibliography}{00}
	...
	\bibitem{kozlinskiy}Outer Tracker calibration and open charm production cross section measurement at LHCb. Kozlinskiy A.V. 2013. 
	...
	\bibitem{garfieldpp} Garfield++ is a toolkit for the detailed simulation of particle detectors that use gas and semi-conductors as sensitive medium. http://garfieldpp.web.cern.ch/garfieldpp/

	
	\bibitem{wouter} Time Calibration of Hera-B the Outer Tracker Chambers in a Cosmic Ray Setup  at NIKHEFF, W. Hulsbergen, O. Steinkamp,  G. van Apeldorn, M. Brruinsma and W. Ruckstuhl. January 2000.
	
	\bibitem{Particle_Detection} Particle Detection with Drift Chambers, Walter Blum, Werner Riegler, Luigi Rolandi. 2008 Springer-Verlag Berlin Heidelberg
	
	\bibitem{escape_peak} GASEOUS DETECTORS FUNDAMENTS, Fabio Sauli, CERN Geneva Switzerland, 2011.
	
	\bibitem{escape2} Thermal Stretching of GEM Foils and Characterization of Triple-GEM Detectors  for Uses in Astronomy, Bryant Benson, april, 2011.
\end{thebibliography}
